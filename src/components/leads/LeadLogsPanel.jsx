...
import { useLeadLogs } from '../../hooks/useLeadLogs';
...
const LeadLogsPanel = ({ leadId, currentPhase }) => {
  const { currentUser, userProfile } = useAuth();
  const { logs, loading } = useLeadLogs(leadId);
  const [newLog, setNewLog] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleSubmitLog = async (e) => {
    e.preventDefault();
    if (!newLog.trim() || !leadId) return;
    ...
    <div className="flex flex-col h-full bg-gray-50 rounded-2xl border border-gray-100 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {loading ? (
          ...
          logs.map(log => (
            <div key={log.logId} className="flex gap-3 animate-in fade-in slide-in-from-bottom-2">
